...
def test_spsc_queue_capacity():
    ...
    update = cpp_core.StrategyUpdate(2.0, 0.0, 0)
...
def test_pydantic_boundary_guardrails(spread, skew, regime):
    ...
    if not (is_valid_spread and is_valid_skew and is_valid_regime):
        # We expect a validation error if the AI hallucinates outside physical bounds
        with pytest.raises(ValidationError):
            AgentUpdate(spread_bps=spread, skew_bps=skew, regime_id=regime)
